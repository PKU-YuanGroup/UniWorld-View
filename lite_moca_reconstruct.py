...
def static_reconstruct(ws, log_path, fit_cfg, init_cam_pth: str | None = None):
    ...
    if init_cam_pth is not None:
        # External init (e.g. STream3R cameras saved by demo.py) takes
        # precedence over both GT init and FOV search.
        logging.info(f"Initializing cameras from external checkpoint: {init_cam_pth}")
        cams = MonocularCameras.load_from_ckpt(
            torch.load(init_cam_pth, map_location="cpu", weights_only=False)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional path to a MonocularCameras state_dict (.pth) used as "
            "initialization. When given, skips the FOV-search init inside "
            "moca_solve (and ignores fit_cfg.init_gt_camera)."
        ),
    )
    ...
    static_reconstruct(args.ws, logdir, cfg, init_cam_pth=args.init_cam_pth)
    dst = osp.join(args.ws, "bundle")
    if osp.exists(dst):
        shutil.rmtree(dst)
    shutil.move(osp.join(logdir, "bundle"), dst)
