...
# align depth with VDA (lazy-imported; avoids forcing NVIDIA CUDA JIT on Ascend/CPU)
VideoDepthAnythingDepthModel = None
DepthEstimationInput = None
unpack_optional = None
align_inv_depth_to_depth = None

def _ensure_vda_imports():
    """Import vipe VDA helpers only when --align_with_vda is enabled."""
    global VideoDepthAnythingDepthModel, DepthEstimationInput, unpack_optional, align_inv_depth_to_depth
    if VideoDepthAnythingDepthModel is not None:
        return
    from vipe.priors.depth.videodepthanything import VideoDepthAnythingDepthModel as _VDA
    from vipe.priors.depth import DepthEstimationInput as _DEI
    from vipe.utils.misc import unpack_optional as _UO
    from vipe.priors.depth.alignment import align_inv_depth_to_depth as _ALIGN
    VideoDepthAnythingDepthModel = _VDA
    DepthEstimationInput = _DEI
    unpack_optional = _UO
    align_inv_depth_to_depth = _ALIGN
...
from utils.device_compat import (
    amp_autocast,
...
class UniScene:
    def __init__(self, opts, gradio: bool = False):
        ...
        # Wan-14B pipeline and aux models cannot reside on NPU together (~61GB HBM).
        self.offload_aux_modules_to_cpu()
        self.offload_pipeline_to_cpu()
    ...
    def setup_sam2(self):
        if self.sampredictor is None:
            ...
            # build_sam2_video_predictor defaults to device="cuda"; pass opts.device for Ascend NPU.
            device_str = str(getattr(self.opts, "device", "cpu"))
            self.sampredictor = build_sam2_video_predictor(
                model_cfg, checkpoint, device=device_str
            )
    ...
    def setup_diffusion(self):
        ...
        # Lower VAE decode peak memory (spatial tiling) — important when Wan-14B just filled HBM.
        try:
            self.pipeline.vae.enable_tiling()
        except Exception:
            pass

        device_str = str(getattr(self.opts, "device", "npu:0"))
        ...
        elif device_str.startswith("npu") or bool(getattr(self.opts, "low_gpu_memory_mode", False)):
            # Keep diffusion weights on CPU; ensure_pipeline_on_device() moves them when needed.
            # Diffusers' enable_model_cpu_offload is CUDA-oriented and unreliable on Ascend NPU.
    ...
    def nvs_single_view(self, image=None):
        ...
        with torch.no_grad(), amp_autocast(self.opts.device, enabled=False):
        ...
        with self.aux_modules_offloaded():
            with torch.no_grad():
                ...
                sample = self.pipeline(
                    ...
                    generator=make_generator(self.opts.device, 42),
                ).frames[0]
    ...
    def _safe_move_module(self, module, device):
        ...
        if isinstance(device, torch.device):
            if device.type in ("cuda", "npu") and device.index is not None:
    ...
    def offload_pipeline_to_cpu(self):
        pipeline = getattr(self, "pipeline", None)
        if pipeline is not None:
            self._safe_move_module(pipeline, "cpu")
        empty_cache()

    def ensure_pipeline_on_device(self, device: torch.device | str | None = None):
        pipeline = getattr(self, "pipeline", None)
        if pipeline is None:
            return
        self._safe_move_module(pipeline, device or self.opts.device)
    ...
    def active_aux_modules(self, device: torch.device | str | None = None):
        # Mutual exclusion: free NPU HBM occupied by Wan-14B before loading aux models.
        self.offload_pipeline_to_cpu()
    ...
    def aux_modules_offloaded(self, device: torch.device | str | None = None):
        # Mutual exclusion: free aux models before bringing Wan-14B pipeline onto NPU.
        self.offload_aux_modules_to_cpu()
        self.ensure_pipeline_on_device(device)
        try:
            yield
        finally:
            self.offload_pipeline_to_cpu()
    ...
    def _nvs_dynamic_view_impl(self):
        ...
        # Keep only STream3R on-device for this stage to minimize HBM peak.
        for module in (
            getattr(self, "depth_model", None),
            getattr(self, "captioner", None),
            getattr(self, "seg_net", None),
            ...
            self._safe_move_module(module, "cpu")
        self._safe_move_module(self.stream3r, device)
        empty_cache()
        ...
        # Free STream3R weights ASAP; restore other aux models for later stages.
        del imgs_stream
        self._safe_move_module(self.stream3r, "cpu")
        empty_cache()
        for module in (
            getattr(self, "depth_model", None),
            getattr(self, "captioner", None),
            ...
            self._safe_move_module(module, device)
        ...
        # imgs_stream already freed right after STream3R; only drop remaining outputs here.
        del outputs
        empty_cache()
        ...
        if self.opts.advanced_render:
            ...
            with torch.inference_mode(), amp_autocast(device, dtype=torch.bfloat16):            
