...
def scaffold_reconstruct(ws, log_path, fit_cfg):
    ...
    ws_name = ws.split("/")[-1]
    cams: MonocularCameras = MonocularCameras.load_from_ckpt(
        torch.load(osp.join(ws, "bundle", "bundle_cams.pth"))
    ).to(device)
...
def photometric_reconstruct(ws, log_path, fit_cfg):
    ...
    photo_solver.photometric_fit(
        s2d=s2d,
        s2d_add=[
            Saved2D(ws).load_rgb("left/images_0").load_sup_mask("left/sam_0/masks.mp4").to(device),
            Saved2D(ws).load_rgb("right/images_0").load_sup_mask("right/sam_0/masks.mp4").to(device),
            Saved2D(ws).load_rgb("left/images_1").load_sup_mask("left/sam_1/masks.mp4").to(device),
            Saved2D(ws).load_rgb("right/images_1").load_sup_mask("right/sam_1/masks.mp4").to(device),
            Saved2D(ws).load_rgb("left/images_2").load_sup_mask("left/sam_2/masks.mp4").to(device),
            Saved2D(ws).load_rgb("right/images_2").load_sup_mask("right/sam_2/masks.mp4").to(device),
            Saved2D(ws).load_rgb("left/images_3").load_sup_mask("left/sam_3/masks.mp4", True).to(device),
            Saved2D(ws).load_rgb("right/images_3").load_sup_mask("right/sam_3/masks.mp4", True).to(device),
            ],
        ...
        cams=cams,
        cams_add=[
            MonocularCameras.load_from_poses(
            ws+'/left/poses_sup0.json',
        ).to(device),
            MonocularCameras.load_from_poses(
            ws+'/right/poses_sup0.json',
        ).to(device),
            MonocularCameras.load_from_poses(
            ws+'/left/poses_sup1.json',
        ).to(device),
            MonocularCameras.load_from_poses(
            ws+'/right/poses_sup1.json',
        ).to(device),
            MonocularCameras.load_from_poses(
            ws+'/left/poses_sup2.json',
        ).to(device),
            MonocularCameras.load_from_poses(
            ws+'/right/poses_sup2.json',
        ).to(device),
            MonocularCameras.load_from_poses(
            ws+'/left/poses_sup3.json',
        ).to(device),
            MonocularCameras.load_from_poses(
            ws+'/right/poses_sup3.json',
        ).to(device),
        ],
        s_model=s_model,
        d_model=d_model,
