...
class UniViewReferenceCrossAttention(nn.Module):
    def __init__(
        self,
        dim: int,
        num_heads: int,
        ...
        super().__init__()
        self.norm_kv = nn.LayerNorm(dim, eps=eps, elementwise_affine=True)
        self.norm_q = nn.LayerNorm(dim, eps=eps, elementwise_affine=True)
        self.attn = Attention(
            query_dim=dim,
            heads=num_heads,
            kv_heads=num_heads,
    ...
    def forward(self, query_states: torch.Tensor, ref_tokens: torch.Tensor) -> torch.Tensor:
        query_states = self.norm_q(query_states)
        ref_tokens = self.norm_kv(ref_tokens)
        return self.attn(hidden_states=query_states, encoder_hidden_states=ref_tokens)
    ...
    def __call__(
        ...
        if rotary_emb is not None:
            ...
            def apply_rotary_emb(hidden_states: torch.Tensor, freqs: torch.Tensor):
                # Ascend NPU does not support complex128 (from float64); use float32 -> complex64.
                # MPS similarly prefers float32.
                if hidden_states.device.type in ("mps", "npu"):
                    dtype = torch.float32
                else:
                    dtype = torch.float64
...
class WanRotaryPosEmbed(nn.Module):
    def __init__(
        ...
        freqs = []
        # float64 -> complex128 is unsupported on Ascend NPU; use float32 -> complex64 there (same as MPS).
        use_fp32_rope = torch.backends.mps.is_available()
        try:
            if hasattr(torch, "npu") and torch.npu.is_available():
                use_fp32_rope = True
        except Exception:
            pass
        freqs_dtype = torch.float32 if use_fp32_rope else torch.float64
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # Build rotary freqs on CPU, then move once — avoids NPU complex128 cat and reduces HBM peak.
        freqs = self.freqs
        if hidden_states.device.type == "npu" and freqs.dtype == torch.complex128:
        ...
        return freqs.to(device=hidden_states.device)
...
class WanVACETransformer3DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin, CacheMixin):
    ...
    def __init__(
        ...
        uniview_ref_in_channels: int = 16,
        uniview_ref_inject_vace_block_indices: Optional[List[int]] = None,
        uniview_ref_num_inject: int = 4,
        uniview_ref_gate_init: float = 1e-3,
    ) -> None:
        super().__init__()
        ...
        self.ref_patch_embedding = nn.Conv3d(
            uniview_ref_in_channels, inner_dim, kernel_size=patch_size, stride=patch_size
        )
        with torch.no_grad():
            if (
                self.ref_patch_embedding.weight.shape == self.patch_embedding.weight.shape
                and self.ref_patch_embedding.bias is not None
                and self.patch_embedding.bias is not None
                and self.ref_patch_embedding.bias.shape == self.patch_embedding.bias.shape
            ):
                self.ref_patch_embedding.weight.copy_(self.patch_embedding.weight)
                self.ref_patch_embedding.bias.copy_(self.patch_embedding.bias)
        ...
        if uniview_ref_inject_vace_block_indices is None:
            uniview_ref_inject_vace_block_indices = [0, 2, 4, 6]

        valid_inject_indices = [i for i in uniview_ref_inject_vace_block_indices if 0 <= i < len(vace_layers)]
        if len(valid_inject_indices) == 0:
            self.uniview_ref_attn = nn.ModuleList([])
            self.uniview_ref_gates = nn.Parameter(torch.zeros(0))
            self._uniview_ref_inject_map: Dict[int, int] = {}
        else:
            num_inject = min(uniview_ref_num_inject, len(valid_inject_indices))
            valid_inject_indices = valid_inject_indices[:num_inject]
            self.uniview_ref_attn = nn.ModuleList(
                [
                    UniViewReferenceCrossAttention(
                        dim=inner_dim,
            ...
            self.uniview_ref_gates = nn.Parameter(torch.full((num_inject,), float(uniview_ref_gate_init)))
            self._uniview_ref_inject_map = {vace_block_idx: i for i, vace_block_idx in enumerate(valid_inject_indices)}
    ...
    def forward(
        ...
        has_ref = ref_latents is not None
        ...
        if has_ref:
            ref_tokens = self.ref_patch_embedding(ref_latents)
        ...
        if torch.is_grad_enabled() and self.gradient_checkpointing:
            ...
            for i, block in enumerate(self.vace_blocks):
                ...
                if conditioning_states is not None and ref_tokens is not None and i in self._uniview_ref_inject_map:
                    ref_idx = self._uniview_ref_inject_map[i]
                    delta = self.uniview_ref_attn[ref_idx](conditioning_states, ref_tokens)
                    conditioning_states = conditioning_states + self.uniview_ref_gates[ref_idx].to(
    ...
    def from_pretrained(
        ...
        print(f"[SKIP] loading 3D transformer's pretrained weights from {pretrained_model_path} (commented out)")
        ...
        # ---- commented out: load pretrained weights from checkpoints/UniView ----
        ...
        # print(m)
        #
        # # If stage-1 checkpoints don't contain UniView reference branch weights,
        # # initialize the reference patch embedding from the (now-loaded) main patch embedding.
        # if any(k.startswith("ref_patch_embedding.") for k in m):
        #     try:
        #         with torch.no_grad():
        #             if model.ref_patch_embedding.weight.shape == model.patch_embedding.weight.shape:
        #                 model.ref_patch_embedding.weight.copy_(model.patch_embedding.weight)
        #                 if (
        #                     model.ref_patch_embedding.bias is not None
        #                     and model.patch_embedding.bias is not None
        #                     and model.ref_patch_embedding.bias.shape == model.patch_embedding.bias.shape
        #                 ):
        #                     model.ref_patch_embedding.bias.copy_(model.patch_embedding.bias)
        #     except Exception as e:
        #         print(f"Warning: failed to init ref_patch_embedding from patch_embedding due to: {e}")
        #
        ...
        # ---- end commented out weight loading ----

        # Build model from config only (no checkpoint weight load).
        model = cls.from_config(config, **transformer_additional_kwargs)
