...
def build_cameras(cam_traj, w2c_0, c2w_0, intrinsic, nframe, focal_length,
    ...
    if w2c_0.ndim == 2:
        print('w2c_0.ndim == 2')
        w2c_0 = w2c_0[None].repeat(nframe, 1, 1)
    if c2w_0.ndim == 2:
        print('c2w_0.ndim == 2')
        c2w_0 = c2w_0[None].repeat(nframe, 1, 1)
    if intrinsic.ndim == 2:
        print('intrinsic.ndim == 2')
    ...
    if cam_traj == "rel_target":
        for i in range(nframe):
            coef = 1
            d_thetas.append(d_theta * coef)
            d_phis.append(d_phi * coef)
            d_rs.append(coef * d_r + (1 - coef) * 1.0)
            x_offsets.append(radius * x_offset)
            y_offsets.append(radius * y_offset)
            z_offsets.append(radius * z_offset)
    elif cam_traj == "free":
    ...
    for i in range(nframe):
        ...
        center = c2w_0[i][:3, 3] + c2w_0[i][:3, 2] * radius

        T_to_origin = torch.eye(4, dtype=torch.float32)
        T_to_origin[:3, 3] = -center

        T_back = torch.eye(4, dtype=torch.float32)
        T_back[:3, 3] = center

        c2w_1 = T_back @ R_phi @ R_theta @ T_to_origin @ c2w_0[i]

        if i < len(x_offsets) and i < len(y_offsets) and i < len(z_offsets):
            c2w_1[:3, 3] += torch.tensor([x_offsets[i], y_offsets[i], z_offsets[i]])
        c2w_1[:3, 3] *= d_rs[i]
        ...
        intrinsic[i, :2, :2] = intrinsic[i, :2, :2] * focal_length * (i / nframe) + \
                            intrinsic[i, :2, :2] * ((nframe - i) / nframe)
