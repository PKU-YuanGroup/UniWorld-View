...
def save_images_folder(frames_np, folder_path):
    if os.path.exists(folder_path):
        shutil.rmtree(folder_path)
    os.makedirs(folder_path)
    for i in range(frames_np.shape[0]):
        image_cur = Image.fromarray((frames_np[i]*255).astype(np.uint8))
        image_cur.save(os.path.join(folder_path, f"{i:05d}.png"))

def read_video_frames(video_path, process_length, stride, max_res, dataset="open", width: int | None = None, height: int | None = None):
    if dataset == "open":
