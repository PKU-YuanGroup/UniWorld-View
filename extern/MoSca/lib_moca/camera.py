...
class MonocularCameras(nn.Module):
    ...
    @classmethod
    def load_from_poses(cls, pose_path):
        # load camera poses
        
        cam_poses = json.load(open(pose_path))
        logging.info("Load camera from poses file")
        H = cam_poses["H"]
        W = cam_poses["W"]
        delta_flag = False
        T = len(cam_poses["pose"])
        K = np.array(cam_poses["K"][0])
        init_camera_pose = np.array(cam_poses["pose"])
        cams = cls(n_time_steps=T, default_H=H, default_W=W, delta_flag=delta_flag, K=K, init_camera_pose=init_camera_pose)
        return cams
