...
class DynReconstructionSolver:
    ...
    def photometric_fit(
        self,
        s2d: Saved2D = None,
        s2d_add: list[Saved2D] = None,
        cams: MonocularCameras = None,
        cams_add: list[MonocularCameras] = None,
        s_model: StaticGaussian = None,
        ...
        for step in tqdm(range(total_steps)):
            ...
            cams.zero_grad()
            #cams_add = [cams_cur.zero_grad() for cams_cur in cams_add]
            s_model.zero_grad()
            s2d.zero_grad()
            #s2d_add = [s2d_cur.zero_grad() for s2d_cur in s2d_add]
            if d_flag:
            ...
            for _inner_loop_i, view_ind in enumerate(view_ind_list):
                ...
                if s2d_add is not None and len(s2d_add) > 0:
                    add_index = np.random.randint(len(s2d_add))

                    render_dict_add = render(
                        gs5,
                        s2d.H,
                        s2d.W,
                        cams_add[add_index].K(s2d.H, s2d.W),
                        cams_add[add_index].T_cw(view_ind),
                        bg_color=bg_color,
                        add_buffer=add_buffer,
                    )
                    render_dict_list.append(render_dict_add)
                ...
                if s2d_add is not None and len(s2d_add) > 0:
                    rgb_sup_mask_add = s2d_add[add_index].get_mask_by_key("sup")[view_ind]
                    dep_sup_mask = rgb_sup_mask
                    _l_rgb_add, _, _, _ = compute_rgb_loss(
                        s2d_add[add_index].rgb[view_ind].detach().clone(), render_dict_add, rgb_sup_mask_add
                    )
                    _l_rgb += _l_rgb_add
