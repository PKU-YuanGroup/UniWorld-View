...
class DepthCrafterDemo:
    def __init__(
        ...
        if cpu_offload is not None:
            ...
            self.pipe.to("cuda")
        # NB: xformers 0.0.27 + H20 (Hopper sm_90) + torch 2.4 + cu124 hits
        # an "invalid configuration argument" inside its flash-attention
        # forward kernel. Skip xformers entirely and rely on diffusers'
        # default attention (PyTorch 2.x SDPA, which natively supports sm_90).
        # If you are on a different GPU + xformers combo, swap the
        # `False` below back to True.
        use_xformers = False
        if use_xformers:
            try:
