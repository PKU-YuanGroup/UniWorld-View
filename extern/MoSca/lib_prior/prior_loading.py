...
class Saved2D(nn.Module):
    ...
    @torch.no_grad
    def load_sup_mask(self, rgb_dirname="sam/masks.mp4", use_all=False):
        # read mask video
        cap = cv2.VideoCapture(osp.join(self.ws, rgb_dirname))
        mask_frames = []
        while True:
            ret, frame = cap.read()
            if not ret:
            ...
            mask_frames.append(frame[..., 0]>=128)
        masks = torch.from_numpy(np.stack(mask_frames)).squeeze().float()
        if use_all:
            masks = torch.ones_like(masks)
        self.register_gradfree_buffer("sup_mask", masks.detach())
        return self
