...
class MoCaPrep:
    def __init__(
        ...
        # ckpt_path
        flow_ckpt="./checkpoints/mosca/raft_models/raft-things.pth",
        tap_ckpt="./checkpoints/mosca/spaT_final.pth",
        # depthcrafter ckpt_path (HF repo id or local dir)
        depthcrafter_unet_path="tencent/DepthCrafter",
        depthcrafter_pretrain_path="stabilityai/stable-video-diffusion-img2vid-xt",
        # metric3d ckpt_path (HF repo id, or local .pth path)
        metric3d_ckpt_path=None,
        metric3d_torch_hub_path=None,
    ):
        ...
        self.__init_flow__(flow_mode, device, flow_ckpt)
        ...
        self.__init_depth__(
            ...
            depthcrafter_pretrain_path=depthcrafter_pretrain_path,
            metric3d_ckpt_path=metric3d_ckpt_path,
            metric3d_torch_hub_path=metric3d_torch_hub_path,
        )
        self.__init_tap__(
            tap_mode,
            device,
    ...
    def __init_flow__(self, flow_mode, device, ckpt_path):
    ...
    def __init_depth__(
        ...
        if self.dep_mode == "metric3d":
            ...
            self.depth_model = get_metric3dv2_model(
                device=device,
                version=metric3d_depth_version,
                ckpt_path=metric3d_ckpt_path,
                torch_hub_path=metric3d_torch_hub_path,
            )
            ...
            self.depth_model = get_depthcrafter_model(
                unet_path=depthcrafter_unet_path,
                pre_train_path=depthcrafter_pretrain_path,
                modify_scheduler=True,
            )
    ...
    def __init_tap__(
        ...
        if tap_mode == "spatracker":
            ...
            self.tap = get_spatracker(device, S_lenth=spatracker_S_lenth, ckpt_path=ckpt_path)
    ...
    def compute_depth(
        ...
        def _depth_already_done(dst, expected_fns):
            """Return True iff `dst` already contains an .npz per expected
            frame basename — in that case the caller can skip regenerating
            depth (which is expensive for depthcrafter / metric3d)."""
            ...
            return expected.issubset(on_disk) and len(on_disk) >= len(expected)
        ...
        if self.dep_mode == "metric3d":
            ...
            if _depth_already_done(depth_save_dir, fn_list):
                logging.warning(
                    f"depth_save_dir {depth_save_dir} already populated, skip running metric3d!"
                )
            else:
            ...
            depthcrafter_dst = depth_save_dir + "_raw_disp"
            if _depth_already_done(depthcrafter_dst, fn_list):
                logging.warning(
                    f"depth_save_dir {depthcrafter_dst} already populated, skip running depthcrafter!"
                )
            else:
                self.depth_process_func(
                    self.depth_model,
            ...
            if _depth_already_done(depth_save_dir, fn_list):
                logging.warning(
                    f"depth_save_dir {depth_save_dir} already populated, skip running uni-depth!"
                )
            else:
                self.depth_process_func(
                    self.depth_model,
        ...
        if self.dep_mode in ["depthcrafter"]:
            if self.align_to_metric:
                ...
                if self.align_mode in ["metric3d", "uni"]:
                    ...
                    if _depth_already_done(align_dir, align_fns):
                        logging.warning(
                            f"align_dir {align_dir} already populated, skip running alignment metric3d!"
                        )
                    else:
