...
def get_uniform_random_queries(
    ...
    if mask_list is not None:
        ...
        # Guard against the all-zero case: if every frame has no valid
        # pixels (e.g. empty depth mask), fall back to uniform weighting
        # instead of producing NaNs from 0/0.
        _count_total = _count.sum()
        if _count_total > 0:
            mask_weight = _count / _count_total
        else:
            mask_weight = torch.ones_like(_count) / _count.numel()
