...
def load_model_cfg(cfg, saved_dir, device=torch.device("cuda")):
    ...
    '''cams = MonocularCameras.load_from_poses(
        '/mnt/workspace/zhy/TuLing/SceneCrafter_new/output/9/left/poses_sup3.json'
    ).to(device)'''
    '''cams = MonocularCameras.load_from_poses(
        '/mnt/workspace/zhy/TuLing/SceneCrafter_new/output/9/left/poses_sup3.json'
    ).to(device)'''
...
def viz_main(
    ...
    for key_time_step in key_steps:
        ...
        horizontal_pose_list = [cams.T_cw(key_time_step) for _ in range(cams.T)]
        for i in range(cams.T):
            horizontal_pose_list[i][0, 3] -= i/cams.T
        ...
        save_fn_prefix = osp.join(save_dir, f"{key_time_step}_horizontal_moving")
        viz_single_2d_video(
            H,
            W,
            cams,
...
if __name__ == "__main__":
    ...
    args.add_argument("--N", "-n", type=int, default=3)
